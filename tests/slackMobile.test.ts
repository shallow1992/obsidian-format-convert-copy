...
describe("Slack Mobile HTML Converter (iOS / Android)", () => {
	...
	describe("Document Conversion (convertToSlackMobileHtml)", () => {
		...
		it("does not let prose containing $$ interfere with block math or heading formatting", () => {
			...
			// 2. Inline formulas are wrapped in <code> tags
			expect(html).toContain("<code>$E = mc^2$</code>");
			expect(html).toContain("<code>$x_1 * y_1 + a_n$</code>");

			// 3. Block formulas are wrapped in backtick code block paragraphs
			expect(html).toContain("<p>```<br>$$<br>E = \\sqrt{(mc^2)^2 + (pc)^2}<br>$$<br>```</p>");
			expect(html).toContain("<p>```<br>$$<br>\\sum_{i=1}^{n} x_i * y_i = \\int_{-\\infty}^{\\infty} \\hat{f}(\\xi)\\,e^{2 \\pi i \\xi x}\\,d\\xi<br>$$<br>```</p>");
